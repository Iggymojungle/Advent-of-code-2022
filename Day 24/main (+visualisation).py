...
def draw_grid(grid, coords, pg_grid):
    for line_num, line in enumerate(grid):
        for cell_num, cell in enumerate(line):
            if [line_num, cell_num] in coords:
                pg_grid[cell_num][line_num].colour = (22, 224, 76)
            elif cell == "#":
                pg_grid[cell_num][line_num].colour = (55, 184, 240)
            elif cell == ".":
                pg_grid[cell_num][line_num].colour = (255, 255, 255)
            else:
                pg_grid[cell_num][line_num].colour = (0, 255, 255)
    pg_grid.draw()


def bfs(grid, coords, end, pg_grid):
    last_print = 0
    steps = 0
    queue = [coords]
    grid_cache = [grid]
...
def main():
    ...
    grid = get_data()
    coords = [0, 1]
    pg_grid = pygrille.Grid(PIXEL_SIZE, [len(grid[0]), len(grid)], framerate = 60)
    coords, steps0, grid = bfs(grid, coords, [len(grid) - 1, len(grid[0]) - 2], pg_grid)
    if not coords:
        return
    print(f"Part 1: {steps0}")
    coords, steps1, grid = bfs(grid, coords, [0, 1], pg_grid)
    if not coords:
        return
    coords, steps2, grid = bfs(grid, coords, [len(grid) - 1, len(grid[0]) - 2], pg_grid)
    if not coords:
        return
    print(f"Part 2: {steps0 + steps1 + steps2 + 2}")
    end = perf_counter()
